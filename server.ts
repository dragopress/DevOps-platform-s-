...
*Note: Architecture review generated using DevSecOps Policy Engine.*`;
  ...
  // AI Vulnerability Quick-Patch Remediation endpoint
  app.post("/api/ai/generate-remediation", async (req, res) => {
    try {
      const apiKey = process.env.GEMINI_API_KEY;
      const { cveId, cwe, title, severity, scanner, asset, packageName, installedVersion, fixedVersion, patchType } = req.body;
      ...
      const systemInstruction = `You are a Lead DevSecOps & Cloud Security Architect.
Generate a concise, ready-to-apply ${patchType === "dockerfile" ? "Dockerfile" : "Terraform (.tf)"} remediation code snippet for the following vulnerability.
Target format: ${patchType === "dockerfile" ? "Dockerfile syntax" : "Terraform HCL syntax"}.
Return JSON format with two keys:
- "snippet": code snippet string without markdown fences
- "explanation": 2-sentence technical summary of what this patch remediates and why it works.`;

      const promptText = `Remediate vulnerability:
CVE/ID: ${cveId} (${cwe || 'CWE-Security'})
Title: ${title}
Severity: ${severity}
Scanner: ${scanner}
Target Asset: ${asset}
Package: ${packageName ? `${packageName} (${installedVersion} -> ${fixedVersion})` : 'N/A'}
Patch Format Requested: ${patchType}`;
      ...
      for (const modelCandidate of modelsToTry) {
        try {
          ...
          console.warn(`Gemini model '${modelCandidate}' failed on remediation generation. Trying fallback...`);
        }
      }
      ...
      if (responseText) {
        try {
          ...
          return res.json({ snippet: parsed.snippet, explanation: parsed.explanation });
        } catch {
          return res.json({ snippet: responseText, explanation: "AI Architect generated remediation patch." });
        }
      }

      return res.status(503).json({ error: "Gemini API unavailable for remediation snippet generation." });
    } catch (err: any) {
      console.error("Remediation endpoint error:", err);
      res.status(500).json({ error: err.message || "Failed to generate remediation patch." });
    }
  });
